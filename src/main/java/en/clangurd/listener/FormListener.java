...
public final class FormListener implements Listener {
    ...
    private void handleMainMenu(Player player, FormWindowSimple window) {
        ...
        switch (response.getClickedButtonId()) {
            ...
            case 7: handleChangeNameButton(player); break;
            case 8: handleDescriptionButton(player); break;
            case 9: handleDeleteClanButton(player); break;
        }
    }

    private void handleClanInfoButton(Player player) {
        String clanName = ClanManager.getPlayerClan(player.getName());
        if (clanName == null) { player.sendMessage(Lang.get("prefix") + Lang.get("error_not_in_clan")); return; }
    ...
    private void handleInviteButton(Player player) {
        String clanName = ClanManager.getPlayerClan(player.getName());
        if (clanName == null) { player.sendMessage(Lang.get("prefix") + Lang.get("error_not_in_clan")); return; }
        if (!ClanManager.isPlayerLeader(player.getName(), clanName)) { player.sendMessage(Lang.get("prefix") + Lang.get("error_not_leader")); return; }
    ...
    private void handleDepositButton(Player player) {
        if (!ClanGuard.isEconomyEnabled()) { player.sendMessage(Lang.get("prefix") + Lang.get("error_economy_disabled")); return; }
        String clanName = ClanManager.getPlayerClan(player.getName());
        if (clanName == null) { player.sendMessage(Lang.get("prefix") + Lang.get("error_not_in_clan")); return; }
    ...
    private void handleWithdrawButton(Player player) {
        ...
        if (treasury <= 0) { player.sendMessage(Lang.get("prefix") + Lang.get("error_treasury_empty")); return; }
    ...
    private void handleCreateClan(Player player, FormWindowCustom window) {
        ...
        if (clanName.isEmpty()) { player.sendMessage(Lang.get("prefix") + Lang.get("error_empty_name")); return; }
        if (!NAME_PATTERN.matcher(clanName).matches()) { player.sendMessage(Lang.get("prefix") + Lang.get("error_invalid_name")); return; }
        if (clanTag.isEmpty()) { player.sendMessage(Lang.get("prefix") + Lang.get("error_empty_tag")); return; }
        if (!TAG_PATTERN.matcher(clanTag).matches()) { player.sendMessage(Lang.get("prefix") + Lang.get("error_invalid_tag")); return; }
        String existingClan = ClanManager.getPlayerClan(player.getName());
        if (existingClan != null) { player.sendMessage(Lang.get("prefix") + Lang.get("error_already_in_clan") + existingClan); return; }
        if (ClanManager.clanExists(clanName)) { player.sendMessage(Lang.get("prefix") + Lang.get("error_clan_exists")); return; }
        if (ClanManager.tagExists(clanTag)) { player.sendMessage(Lang.get("prefix") + Lang.get("error_tag_exists")); return; }
    ...
    private void handleInvite(Player player, FormWindowCustom window) {
        ...
        if (targetName.isEmpty()) { player.sendMessage(Lang.get("prefix") + Lang.get("error_player_offline")); return; }
        Player target = Server.getInstance().getPlayerExact(targetName);
        if (target == null) { player.sendMessage(Lang.get("prefix") + Lang.get("error_player_offline")); return; }
        if (target.getName().equalsIgnoreCase(player.getName())) { player.sendMessage(Lang.get("prefix") + Lang.get("error_cannot_invite_self")); return; }
        if (ClanManager.getPlayerClan(target.getName()) != null) { player.sendMessage(Lang.get("prefix") + Lang.get("error_player_in_clan")); return; }
        if (ClanManager.hasPendingInvite(target.getName())) { player.sendMessage(Lang.get("prefix") + Lang.get("error_invite_pending")); return; }
        String clanName = ClanManager.getPlayerClan(player.getName());
    ...
    private void handleInviteResponse(Player player, FormWindowSimple window) {
        FormResponseSimple response = window.getResponse();
        if (response == null) { ClanManager.removePendingInvite(player.getName()); return; }
        String[] invite = ClanManager.getPendingInvite(player.getName());
        if (invite == null) { player.sendMessage(Lang.get("prefix") + Lang.get("error_invite_expired")); return; }
        ...
        if (response.getClickedButtonId() == 0) {
            ...
            } else { player.sendMessage(Lang.get("prefix") + Lang.get("error_clan_full")); }
        } else {
            ...
            if (leader != null) leader.sendMessage(Lang.get("prefix") + Lang.get("notify_invite_declined", "player", player.getName()));
        }
    }
    ...
    private void handleKickConfirm(Player player, FormWindowSimple window) {
        FormResponseSimple response = window.getResponse();
        if (response == null) { pendingKicks.remove(player.getName()); return; }
        ...
        if (response.getClickedButtonId() == 0) {
            ...
            if (ClanManager.kickMemberFromClan(clanName, memberToKick)) {
                ...
                if (kickedPlayer != null) kickedPlayer.sendMessage(Lang.get("prefix") + Lang.get("notify_kicked"));
            ...
            } else { player.sendMessage(Lang.get("prefix") + Lang.get("error_failed")); }
        } else { player.sendMessage(Lang.get("prefix") + Lang.get("success_kick_cancelled")); }
    }
    ...
    private void handleTreasuryDeposit(Player player, FormWindowSimple window) {
        ...
        if (response == null) return;
        int buttonId = response.getClickedButtonId();
        if (buttonId == 7) { ClanTreasuryCustomDepositGui.open(player); return; }
        if (buttonId >= 0 && buttonId < DEPOSIT_AMOUNTS.length) processDeposit(player, DEPOSIT_AMOUNTS[buttonId]);
    }
    ...
    private void handleCustomDeposit(Player player, FormWindowCustom window) {
        ...
        try {
            ...
            if (amount < 1000) { player.sendMessage(Lang.get("prefix") + Lang.get("error_amount_min")); return; }
            if (amount > 10000000) { player.sendMessage(Lang.get("prefix") + Lang.get("error_amount_max_deposit")); return; }
            processDeposit(player, amount);
        } catch (NumberFormatException e) { player.sendMessage(Lang.get("prefix") + Lang.get("error_amount_invalid")); }
    }
    ...
    private void handleTreasuryWithdraw(Player player, FormWindowSimple window) {
        ...
        if (buttonId >= 0 && buttonId < DEPOSIT_AMOUNTS.length) processWithdraw(player, DEPOSIT_AMOUNTS[buttonId]);
    }
    ...
    private void processWithdraw(Player player, long amount) {
        ...
        if (!ClanManager.isPlayerLeader(player.getName(), clanName)) { player.sendMessage(Lang.get("prefix") + Lang.get("error_not_leader")); return; }
        long treasury = ClanManager.getClanTreasury(clanName);
        if (amount > treasury) { player.sendMessage(Lang.get("prefix") + Lang.get("error_treasury_insufficient") + formatMoney(treasury)); return; }
    ...
    private void handleClanChat(Player player, FormWindowCustom window) {
        ...
        if (message.isEmpty()) { player.sendMessage(Lang.get("prefix") + Lang.get("error_empty_message")); return; }
        if (message.length() > 256) { player.sendMessage(Lang.get("prefix") + Lang.get("error_message_long")); return; }
        String clanName = ClanManager.getPlayerClan(player.getName());
        if (clanName == null) { player.sendMessage(Lang.get("prefix") + Lang.get("error_not_in_clan")); return; }
    ...
    private void handleDeleteClan(Player player, FormWindowSimple window) {
        ...
        } else { player.sendMessage(Lang.get("prefix") + Lang.get("success_delete_cancelled")); }
    }
    ...
    private void handleChangeName(Player player, FormWindowCustom window) {
        ...
        if (newName.isEmpty()) { player.sendMessage(Lang.get("prefix") + Lang.get("error_empty_name")); return; }
        if (!NAME_PATTERN.matcher(newName).matches()) { player.sendMessage(Lang.get("prefix") + Lang.get("error_invalid_name")); return; }
        String currentClan = ClanManager.getPlayerClan(player.getName());
        if (currentClan == null) { player.sendMessage(Lang.get("prefix") + Lang.get("error_not_in_clan")); return; }
        if (!ClanManager.isPlayerLeader(player.getName(), currentClan)) { player.sendMessage(Lang.get("prefix") + Lang.get("error_not_leader")); return; }
        if (ClanManager.clanExists(newName)) { player.sendMessage(Lang.get("prefix") + Lang.get("error_clan_exists")); return; }
        ...
        Server.getInstance().getScheduler().scheduleAsyncTask(plugin, new AsyncTask() {
            @Override public void onRun() { try { Thread.sleep(10000); } catch (InterruptedException ignored) {} }
            @Override public void onCompletion(Server server) {
                boolean success = ClanManager.updateClanName(oldName, newName);
                Player onlinePlayer = server.getPlayerExact(playerName);
                if (onlinePlayer != null) {
                    if (success) onlinePlayer.sendMessage(Lang.get("prefix") + Lang.get("success_clan_renamed", "name", newName));
                    else onlinePlayer.sendMessage(Lang.get("prefix") + Lang.get("error_failed"));
                }
            }
        });
    }
    ...
    private void handleDescription(Player player, FormWindowCustom window) {
        ...
        if (description.isEmpty()) { player.sendMessage(Lang.get("prefix") + Lang.get("error_empty_description")); return; }
        if (description.length() > 200) { player.sendMessage(Lang.get("prefix") + Lang.get("error_description_long")); return; }
        String clanName = ClanManager.getPlayerClan(player.getName());
        if (clanName == null) { player.sendMessage(Lang.get("prefix") + Lang.get("error_not_in_clan")); return; }
        if (!ClanManager.isPlayerLeader(player.getName(), clanName)) { player.sendMessage(Lang.get("prefix") + Lang.get("error_not_leader")); return; }
        player.sendMessage(Lang.get("prefix") + Lang.get("notify_processing"));
        ...
        final String playerName = player.getName();
        Server.getInstance().getScheduler().scheduleAsyncTask(plugin, new AsyncTask() {
            @Override public void onRun() { try { Thread.sleep(5000); } catch (InterruptedException ignored) {} }
            @Override public void onCompletion(Server server) {
                boolean success = ClanManager.updateClanDescription(clan, desc);
                Player onlinePlayer = server.getPlayerExact(playerName);
                if (onlinePlayer != null) {
                    if (success) onlinePlayer.sendMessage(Lang.get("prefix") + Lang.get("success_description_added"));
                    else onlinePlayer.sendMessage(Lang.get("prefix") + Lang.get("error_failed"));
                }
            }
        });
